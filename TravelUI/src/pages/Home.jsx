...
function Home() {
  ...
  const fetchJournals = async () => {
    try {
      const response = await axiosInstance.get("/api/Journal/journals/feed/");
  ...
  useEffect(() => {
    console.log("Home.jsx: Current User:", currentUser); // Debug log
    fetchJournals();
  }, []);
    ...
    <>
      <div className="container mx-auto flex p-4">
        <div className="w-3/4 pr-4">
          <h1 className="text-3xl font-bold mb-4">Welcome to WanderTales</h1>
          <button
            onClick={() => navigate("/create")}
            className="w-full bg-blue-600 text-white p-3 rounded hover:bg-blue-700 mb-6"
          >
            Share your travel stories/plans to travel
          </button>
            ...
            >
              ...
              <div className="mt-4">
                {journal.comments.map((comment) => {
                  console.log(
                    `Home.jsx: Comment ID ${comment.id}, User ID ${comment.user?.id}, Current User ID ${currentUser?.id}`
                  ); // Debug log
                  return (
                    <div key={comment.id} className="border-t pt-2 mt-2">
                      ...
                      {comment.replies.map((reply) => {
                        console.log(
                          `Home.jsx: Reply ID ${reply.id}, User ID ${reply.user?.id}, Current User ID ${currentUser?.id}`
                        ); // Debug log
                        return (
                          <div
                            key={reply.id}
